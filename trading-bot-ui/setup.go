...
// GetEnvVar reads a single environment variable from .env file
func (s *SetupManager) GetEnvVar(key string) string {
	data, err := os.ReadFile(s.envFilePath)
	if err != nil {
		return ""
	}
...
// UpdateEnvVars updates or adds environment variables in .env file
func (s *SetupManager) UpdateEnvVars(updates map[string]string) error {
	...
	content.WriteString("# Binance API Configuration\n")
	content.WriteString("# Generated by Trading Bot\n")
	content.WriteString("# Keep this file secure and never share it\n\n")

	// Write Binance API keys first
	if val, ok := existingVars["BINANCE_API_KEY"]; ok {
		content.WriteString(fmt.Sprintf("BINANCE_API_KEY=%s\n", val))
	}
	if val, ok := existingVars["BINANCE_API_SECRET"]; ok {
		content.WriteString(fmt.Sprintf("BINANCE_API_SECRET=%s\n", val))
	}
	if val, ok := existingVars["BINANCE_API_ENDPOINT"]; ok {
		content.WriteString(fmt.Sprintf("BINANCE_API_ENDPOINT=%s\n", val))
	}

	// Write email settings
	content.WriteString("\n# Email Notification Settings\n")
	if val, ok := existingVars["EMAIL_NOTIFICATIONS_ENABLED"]; ok {
		content.WriteString(fmt.Sprintf("EMAIL_NOTIFICATIONS_ENABLED=%s\n", val))
	}
	if val, ok := existingVars["NOTIFICATION_EMAIL"]; ok {
		content.WriteString(fmt.Sprintf("NOTIFICATION_EMAIL=%s\n", val))
	}
	if val, ok := existingVars["SMTP_HOST"]; ok {
		content.WriteString(fmt.Sprintf("SMTP_HOST=%s\n", val))
	}
	if val, ok := existingVars["SMTP_PORT"]; ok {
		content.WriteString(fmt.Sprintf("SMTP_PORT=%s\n", val))
	}
	if val, ok := existingVars["SMTP_FROM_EMAIL"]; ok {
		content.WriteString(fmt.Sprintf("SMTP_FROM_EMAIL=%s\n", val))
	}
	if val, ok := existingVars["SMTP_PASSWORD"]; ok {
		content.WriteString(fmt.Sprintf("SMTP_PASSWORD=%s\n", val))
	}
	...
	if err := os.WriteFile(s.envFilePath, []byte(content.String()), 0600); err != nil {
